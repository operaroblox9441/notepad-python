...
def saveFile():
	new_file = asksaveasfile(mode = 'w', filetypes = [('Plain text', '.txt')])
	if new_file is None:
		return
...
def openFile():
	file = askopenfile(mode = 'r', filetypes = [('Plain text', '*.txt')])
	if file is not None:
		content = file.read()
...
entry = Text(canvas,wrap = WORD, bg = "#F9DDA4", font = ("roboto", 15))
